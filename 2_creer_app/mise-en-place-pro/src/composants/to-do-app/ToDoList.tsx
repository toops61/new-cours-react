...
import blackholeGif from '../../assets/trou_noir.gif';

export default function ToDoList({fruityRef}:{fruityRef:HTMLElement|null}) {
  ...
  useEffect(() => {
    tasksRef?.current &&
      tasksRef.current.map((e) =>
        console.log(e.children[0].textContent?.length)
      );
  }, [arrayTasks]);
  ...
  return (
    <div className="todo-main">
      ...
      <form className="todo-form" onSubmit={handleSubmit}>
        ...
        />
        {alert ? (
          <p className="alert">Vous devez d'abord ajouter une tâche</p>
        ) : (
          <></>
        )}
        ...
        <button className="add-button">
          {modifiedObject ? "modifier" : "Ajouter"}
        </button>
      </form>
      {arrayTasks.length ? (
      ...
      <button className="blackhole" onClick={() => setBlackhole(!blackhole)}>trou noir dans fruity</button>
      {blackhole && fruityRef ? createPortal(<div className="blackhole-container"><img src={blackholeGif} alt='blackhole' /></div>,fruityRef) : <></>}
    </div>
  );
}
